# main.py — Dostlar Meclisi | Gri Kutu Kopyalanabilir + Hızlı Dinleme + Tek Polling 🔥
...
@app.route('/')
def home():
    return "🔥 Dostlar Meclisi Bot Aktif 🔥"
...
async def listener(event):
    ...
    if len(lines) < 3:
        return  # site + kod + link üçlüsü aranıyor
    ...
    for l in lines:
        if not site_line and re.match(r'^[A-Za-z0-9\-]{3,}$', l):
            ...
            code_line = l
        elif not link and re.search(r'https?://', l):
            link = l
...
def start_bot_polling():
    if polling_started.is_set():
        return
    polling_started.set()
    print("🤖 TeleBot polling başlatılıyor...")
    while True:
        try:
            bot.remove_webhook()
            bot.infinity_polling(skip_pending=True, allowed_updates=["message"], timeout=60)
        except Exception as e:
            print("⚠️ Polling koptu, 5 sn sonra tekrar denenecek:", e)
            time.sleep(5)

# --- ANA ÇALIŞTIRMA ---
async def main():
    await user.start(phone=lambda: input("📞 Numaran (+90): "))
    print("🚀 Telethon client çalışıyor...")
